...
from networks.mad_gan import mmd
from sklearn.metrics import precision_recall_fscore_support
from networks.mad_gan.mod_core_rnn_cell_impl import LSTMCell
...
def discriminator_o(x, hidden_units_d, reuse=False, parameters=None):
    ...
    with tf.variable_scope("discriminator_0") as scope:
        ...
        b_out_D_initializer = tf.constant_initializer(
            value=parameters["discriminator/b_out_D:0"]
        )

        W_out_D = tf.get_variable(
            name="W_out_D", shape=[hidden_units_d, 1], initializer=W_out_D_initializer
        )
        b_out_D = tf.get_variable(
            name="b_out_D", shape=1, initializer=b_out_D_initializer
        )
...
def dis_trained_model(settings, samples, para_path):
    ...
    # normal GAN
    D_t, L_t = discriminator_o(
        T, settings["hidden_units_d"], reuse=False, parameters=parameters
    )
...
def dis_D_model(settings, samples, para_path):
    ...
    T = tf.placeholder(
        tf.float32, [1, settings["seq_length"], settings["num_generated_features"]]
    )
