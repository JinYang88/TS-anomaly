...
from networks.mad_gan.differential_privacy.dp_sgd.dp_optimizer import utils
...
class DPGradientDescentOptimizer(tf.train.GradientDescentOptimizer):
    ...
    def __init__(
        ...
        super(DPGradientDescentOptimizer, self).__init__(
        ...
        if batches_per_lot > 1:
            self._batch_count = tf.Variable(
                1, dtype=tf.int32, trainable=False, name="batch_count"
            )
            var_list = tf.trainable_variables()
            with tf.variable_scope("grad_acc_for"):
                for var in var_list:
                    v_grad_accum = tf.Variable(
                        tf.zeros_like(var),
                        trainable=False,
    ...
    def minimize(self, loss, global_step=None, var_list=None, name=None):
        ...
        if self._batches_per_lot == 1:
            ...
            apply_grads = self.apply_gradients(
        ...
        update_cond = tf.equal(
            tf.constant(0),
            tf.mod(self._batch_count, tf.constant(self._batches_per_lot)),
        )
        ...
        def last_in_lot_op(loss, var_list, global_step):
            ...
            for var, grad in zip(var_list, sanitized_grads):
                ...
                normalized_grad = tf.div(
                    grad_acc_v.assign_add(grad), tf.to_float(self._batches_per_lot)
                )
        ...
        update_op = tf.cond(
            update_cond,
            lambda: last_in_lot_op(loss, var_list, global_step),
