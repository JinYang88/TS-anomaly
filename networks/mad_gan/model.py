...
import json
import sys
from networks.mad_gan.mod_core_rnn_cell_impl import (
    LSTMCell,
)  # modified to allow initializing bias in lstm

# from tensorflow.contrib.rnn import LSTMCell
tf.logging.set_verbosity(tf.logging.ERROR)
import networks.mad_gan.mmd

from networks.mad_gan.differential_privacy.dp_sgd.dp_optimizer import dp_optimizer
from networks.mad_gan.differential_privacy.dp_sgd.dp_optimizer import sanitizer
from networks.mad_gan.differential_privacy.privacy_accountant.tf import accountant
...
def train_epoch(
    ...
    for batch_idx in range(
        0, int(len(samples) / batch_size) - (D_rounds + G_rounds), D_rounds + G_rounds
    ):
...
def GAN_solvers(
    ...
    if dp:
        ...
        ).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
        priv_accountant = None
    G_loss_mean_over_batch = tf.reduce_mean(G_loss)
    G_solver = tf.train.AdamOptimizer().minimize(
        G_loss_mean_over_batch, var_list=generator_vars
    )
...
def generator(
    ...
    with tf.variable_scope("generator") as scope:
        ...
        W_out_G = tf.get_variable(
            name="W_out_G",
            shape=[hidden_units_g, num_signals],
            initializer=W_out_G_initializer,
        )
        b_out_G = tf.get_variable(
            name="b_out_G", shape=num_signals, initializer=b_out_G_initializer
        )
        scale_out_G = tf.get_variable(
            name="scale_out_G",
            shape=1,
...
def discriminator(
    ...
    with tf.variable_scope("discriminator") as scope:
        if reuse:
            scope.reuse_variables()
        if parameters is None:
            W_out_D = tf.get_variable(
                name="W_out_D",
                shape=[hidden_units_d, 1],
                initializer=tf.truncated_normal_initializer(),
            )
            b_out_D = tf.get_variable(
                name="b_out_D", shape=1, initializer=tf.truncated_normal_initializer()
            )

        else:
            W_out_D = tf.constant_initializer(
                value=parameters["discriminator/W_out_D:0"]
            )
            b_out_D = tf.constant_initializer(
        ...
        cell = tf.nn.rnn_cell.BasicLSTMCell(
            num_units=hidden_units_d, state_is_tuple=True, reuse=reuse
        )
        rnn_outputs, rnn_states = tf.nn.dynamic_rnn(
        ...
        logits = (
            tf.einsum("ijk,km", rnn_outputs, W_out_D) + b_out_D
        )  # output weighted sum
