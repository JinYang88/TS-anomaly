...
def load_data(subdataset):
    number = subdataset[8:]
    dataset = {}
    splits = ["train", "test"]
    shuffle = {'train': True, 'test': False}
    train_data_path = './mscred_data/matrix_data_SMD-' + number + '/train_data/'
    test_data_path = './mscred_data/matrix_data_SMD-' + number + '/test_data/'
...
def test(DataLoader, model, subdataset, x_test):
    print("------Testing-------")
    number = subdataset[8:]
    data_get_len = x_test.shape[0]
    index = math.ceil(data_get_len / 10)
    if not os.path.exists('./mscred_data/matrix_data_SMD-' + number + '/reconstructed_data'):
        os.makedirs('./mscred_data/matrix_data_SMD-' + number + '/reconstructed_data')
    reconstructed_data_path = './mscred_data/matrix_data_SMD-' + number + '/reconstructed_data'
    with torch.no_grad():
        for x in DataLoader:
            x = x.to(device)
            x = x.squeeze()
...
def evaluate(subdataset):
    ...
    matrix_data_path = './mscred_data/' + "matrix_data_SMD-" + number + '/'
