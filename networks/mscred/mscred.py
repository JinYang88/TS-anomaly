...
class MSCRED(nn.Module):
    def __init__(self, in_channels_encoder, in_channels_decoder, data_dict, subdataset, x_train, x_test, x_test_labels,
    ...
    def data_preprocessing(self):
        generate_signature_matrix_node(self.data_dict, self.subdataset, self.save_path, self.gap_time, self.win_size)
        generate_train_test_data(self.subdataset, self.x_train, self.x_test, self.save_path, self.step_max,
                                 self.gap_time, self.win_size)
    ...
    def fit(self):
        ...
        torch.save(
            self.state_dict(), "./mscred_data/checkpoints/model-" + self.subdataset + ".pth"
        )
    ...
    def predict(self):
        ...
        self.to(device)
        test(dataLoader["test"], self, self.subdataset, self.x_test, save_dir=self.save_path, gap_time=self.gap_time)
        print("test of %s finished" % self.subdataset)

        anomaly_score = evaluate(self.subdataset, self.save_path, self.thred_b, self.gap_time)
        anomaly_label = self.x_test_labels[
                        self.gap_time - 1 - len(self.x_test) % self.gap_time: self.gap_time - 1 - len(self.x_test) %
                        self.gap_time + len(anomaly_score)
                        ]
