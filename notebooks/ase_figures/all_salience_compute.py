                    ...
                    if not os.path.exists(metrics_dir):
                        ...
                        if len(metrics_dict) != metric_key_num:
                            ...
                            try:
                                ...
                                anomaly_score_train = normalize_score(
                                    score_dict["train"]
                                )
                                ...
                                res_dict[(hash_id, model)][subdataset] = {
...
def compute_salience(score, label, plot=False, ax=None, fig_saving_path=""):
    ...
    overlapping = (n_upper - a_lower if n_upper >= a_lower else 0) / (
        max(a_upper, n_upper) - min(a_lower, n_lower)
    )
    ...
    n_count_ratio = sigmoid(
        len(n_dict["idx"]) / (len(a_dict["idx"]) + len(n_dict["idx"]))
    )
