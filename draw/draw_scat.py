...
salience_csv = (
    pd.read_excel("./data/all_salience.xlsx").drop(index=0).reset_index(drop=True)
)
...
for i in range(len(model_list)):
    ...
    for j in range(len(dataset_list)):
        ...
        temp.append(
            plt.scatter(x, y, s=30, c=model_color[model], marker=dataset_marker[marker])
        )
    plot.append(temp)
...
plt.savefig("./salience_f1_relation.pdf")
